...
\label{par:method}
\begin{itemize}
	\item \textbf{Name:} getEJBObjectStub( Object primaryKey , byte [ ] streamKey )
	\item \textbf{Start Line:} 1877
...
\begin{enumerate} [resume]
	...
	\item \emph{\checkI}: \textbf{Line 1877 contains a tab character (even if it is just at the end of the line)}.
\end{enumerate}
...
\begin{enumerate} [resume]
	\item \emph{\checkL}: \textbf{Line 1894 is blank for no reason}.
...
\begin{enumerate} [resume]
	...
	\item \emph{\checkAG}: \textbf{Declaration at line 1892 should be at the beginning of `\texttt{try}' block}.
\end{enumerate}
...
\begin{enumerate} [resume]
	\item \emph{\checkAH}: \textbf{In each call of the method, the second parameter `\texttt{byte[] streamKey}' is always passed as `\texttt{null}}'.
